...
def login():
    ...
    if form_registro.enviar.data and form_registro.validate_on_submit():
        ...
        novo = Usuario(
            ...
            telefone=form_registro.telefone.data.strip(),
            senha=senha_hash
        )
        db.session.add(novo)
        db.session.commit()
...
def inicio():
    ...
    return render_template("inicio.html", nome=current_user.nome_usuario, personagens=personagens)
...
def ficha(personagem_id):
    ...
    return render_template("ficha.html", personagem=personagem, status=personagem.status, pericias=personagem.pericias, itens=personagem.itens)
